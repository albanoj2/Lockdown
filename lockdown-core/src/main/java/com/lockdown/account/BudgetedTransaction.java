...
public class BudgetedTransaction extends Transaction {
	...
	public BudgetedTransaction(long id, LocalDate date, String description, String party, Money amount, BudgetEntryMapping mapping) {
		super(id, date, description, party, amount);
		this.mapping = mapping;
	}
	
	public BudgetedTransaction() {
		this(0, LocalDate.now(), "Unnamed transaction", "Unknown", Money.zero(), SingleBudgetEntryMapping.none());
	}
	
	public static BudgetedTransaction now(String description, String party, Money amount, BudgetEntryMapping mapping) {
		return new BudgetedTransaction(0, LocalDate.now(), description, party, amount, mapping);
	}
